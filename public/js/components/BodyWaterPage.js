...
let week = require('../../../json/bodywater/bodywater-week.json');
let month = require('../../../json/bodywater/bodywater-month.json');
let year = require('../../../json/bodywater/bodywater-year.json');
let users = require('../../../json/users.json');
 
export default class BodyWaterPage extends React.Component {
   constructor(props) {
    super(props);
    this.state = {
      user: {},
  ...
  componentDidMount() {
        ...
        this.setState({user: user[0], current: user[0]["bodywater"]});
    }
  render() {
    ...
    return (
      <div>
        <h1>{this.state.user.firstName}'s Current Body Water Percentage: {this.state.current}</h1>;
        {tooltip}
  ...
  selected(){
    return (<h1>{this.state.user.firstName}'s Body Water Percentage at {this.state.x} is {this.state.y}</h1>);
  }
  addZero(i) {
    if(i >= 60){
      i = i % 60;
